/*  vw_sales_summary  (Databricks SQL)
    ─────────────────
...
CREATE OR REPLACE VIEW ${catalog}.retail.vw_sales_summary AS
SELECT
    -- Date
...
FROM      ${catalog}.retail.orders           o
JOIN      ${catalog}.retail.order_lines      ol  ON ol.ord_id    = o.ord_id
JOIN      ${catalog}.retail.products         p   ON p.prod_id    = ol.prod_id
JOIN      ${catalog}.retail.product_categories pc ON pc.prod_cat_id = p.prod_cat_id
JOIN      ${catalog}.retail.stores           s   ON s.str_id     = o.str_id
JOIN      ${catalog}.retail.regions          r   ON r.reg_id     = o.reg_id;
