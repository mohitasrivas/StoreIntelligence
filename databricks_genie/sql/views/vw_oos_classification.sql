/*  vw_oos_classification  (Databricks SQL)
    ──────────────────────
    ...
    NOTE: Replace ${catalog} with your Unity Catalog name before running.
          Run vw_osa_daily, vw_sales_summary, vw_inventory_health first.
*/

CREATE OR REPLACE VIEW ${catalog}.retail.vw_oos_classification AS
WITH
...
avg_velocity AS (
    SELECT
        AVG(CAST(units_sold_7d AS DOUBLE)) AS avg_units_7d
    FROM sales_7d
),
...
inv_exists AS (
    SELECT DISTINCT
        ...
        ih.lead_time_days
    FROM ${catalog}.retail.vw_inventory_health ih
